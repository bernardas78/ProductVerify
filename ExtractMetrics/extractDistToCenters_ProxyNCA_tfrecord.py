...
from ProxyNCA.proxyNcaLayer import ProxyNcaLayer, proxynca_loss
from Globals.globalvars import Glb, MyProxyNcaTfrecordIterator

#experSuffix = "_proxynca_20240222_61234878."   #sco dataset (cosine)
#experSuffix = "_proxynca_20240307_99522849."   #sco dataset (mahh)
#experSuffix = "_proxynca_20240306_25254691."   #sco dataset (eucl)
#experSuffix = "_proxynca_20240309_84777835."   #sco dataset (Mink3)
#experSuffix = "_proxynca_20240310_75653580."   #sco dataset (Mink4)
#experSuffix = "_proxynca_20240227_64178771."    #fruits 360, cosine
experSuffix = "_proxynca_20240312_47399939."     #fruits 360, manh
...
def dists_to_center_cosine(centers, prelast_activations, **kwargs):
    ...
    #print (np.sum (prelast_activations_norm**2, axis=1))
...
# Load model
model_proxynca_filename = os.path.join(Glb.results_folder, "Models", "model"+experSuffix+"h5" )
...
model_proxynca = load_model(model_proxynca_filename, custom_objects={'ProxyNcaLayer': ProxyNcaLayer(distName), 'proxynca_loss': proxynca_loss(distName)})
print ("Loaded")

# proxynca layer
proxynca_layer = model_proxynca.get_layer('proxyncalayer')
print ("proxyncalayer: {}".format(proxynca_layer))

# update minkowski coefficient
if distName=="Minkowski":
    proxynca_layer.p = p_minkowski
    proxynca_layer.get_config()
    print ("proxynca_layer.p: {}".format(proxynca_layer.p))

#if inclInterCenter:
#    print("Lambda2:{}".format(cl_layer.lambda2))

#prelast layer function
for layer_before_proxynca in proxynca_layer._inbound_nodes[0].inbound_layers:
    if (type(layer_before_proxynca).__name__ != 'InputLayer'):
        prelast_layer = layer_before_proxynca
...
assert (centers.shape == (194, 512) or centers.shape == (131, 512))
#print ("centers[2,5]: {}".format (centers[:2,:5]))
#print ("min/max {},{}".format(np.min(centers), np.max(centers)))
#print ("np.sum(centers**2, axis=1): {}".format(np.sum(centers**2, axis=1)))
#print (np.abs(np.sum(centers**2, axis=1)-1.) <1e-6)
#assert ( np.all( np.abs(np.sum(centers**2, axis=1)-1.) <1e-6 ) ) # cosine-centers


#assert (centers.shape==(cnt_classes,prelast_size)) # doesn't work if not pre-last layer

# load data
my_iterator = MyProxyNcaTfrecordIterator(tfrecord_path=tfrecord_filepath, cnt_classes=cnt_classes)
data_yielder = my_iterator.get_iterator_xy_dummy()
...
#    print ( "dist_center_total:{}; pl:{}".format(dist_center_total, pl[i][0]))
#    assert math.isclose(dist_center_total, pl[i][0], rel_tol=1e-4)
