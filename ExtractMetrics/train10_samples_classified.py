...
selected_class_inds = [3,5,14]
selected_class_names = ["Orange","Watermelon","Lemon"]
...
#for batch_id in range(int(my_iterator.len() / 20)):  #full train set will take 4 hours
    ...
    for sample_ind in range(X.shape[0]):
        ...
        if Y_i in selected_class_inds:
            ...
            [ax2.text(0.08, dist_item-1e-2, "{:.2f} {}".format(dist_item, selected_class_names[i])) for (i,dist_item) in enumerate(dists_to_centers_of_selected) ]
